...
import org.evernet.bean.UserAddress;
...
public class Jwt {
    ...
    public AuthenticatedUser getUser(String token) {
        ...
        return AuthenticatedUser.builder()
                ...
                .userNodeIdentifier(issuerNode.getNodeIdentifier())
                .userNodeAddress(issuer)
                .userAddress(UserAddress.builder().nodeAddress(issuerNode).userIdentifier(subject).build().toString())
                .userVertexEndpoint(issuerNode.getVertexEndpoint())
    ...
    public String getUserToken(String userIdentifier, String userNodeIdentifier, String targetNodeAddress, PrivateKey privateKey) {
        String vertexEndpoint = configService.getVertexEndpoint();

        if (targetNodeAddress == null) {
            targetNodeAddress = NodeAddress.builder().vertexEndpoint(vertexEndpoint).nodeIdentifier(userNodeIdentifier).build().toString();
        }

        NodeAddress userNodeAddress = NodeAddress.builder().vertexEndpoint(vertexEndpoint).nodeIdentifier(userNodeIdentifier).build();

        return Jwts.builder().subject(userIdentifier)
                .issuedAt(new Date())
                .id(UUID.randomUUID().toString())
                .claim(TOKEN_TYPE_CLAIM, TOKEN_TYPE_USER)
                .issuer(userNodeAddress.toString())
                .audience().add(targetNodeAddress)
                .and()
                .header().keyId(userNodeAddress.toString())
                .and()
                .signWith(privateKey)
                .compact();
    }
