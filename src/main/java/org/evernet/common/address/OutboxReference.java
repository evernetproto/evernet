...
public class OutboxReference {
    ...
    public String getAddress() {
        return String.format("%s/%s/%s/%s", vertex, nodeIdentifier, "outbox", identifier);
    }


    public static OutboxReference from(String str) {
        String[] components = str.split("/");

        if (components.length != 4) {
            throw new ClientException(String.format("Invalid outbox reference %s", str));
        }
        ...
        return OutboxReference.builder()
                .vertex(components[0])
